...
#include <iostream>
    Renderer::Renderer() 
	    ...
	    EBO1(indicies.data(), indicies.size() * sizeof(GLuint))      // Generates Element Buffer Object and links it to indices
...
void Renderer::renderMesh(std::vector<PackedVector> data, glm::i32vec3 chunkPos) {
	...
	for (const PackedVector& unpackedData : data) {
        ...
        // calculate texture coordinates based on face direction
        float texLeft = 0.0f;
        float texRight = texWidth;
        

        // GLfloat to ensure the float size on all devices
        switch (dir) {
        case UP:
            texLeft = 0.0f * texWidth;
            texRight = 1.0f * texWidth;
            // t    op face (y = blockWorldPos.y) - at the top
            tempVertices = {
                (GLfloat)blockWorldPos.x,              (GLfloat)blockWorldPos.y, (GLfloat)blockWorldPos.z,              texLeft, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y, (GLfloat)blockWorldPos.z,              texRight, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y, (GLfloat)(blockWorldPos.z - fullSize), texRight, 1.0f,
                (GLfloat)blockWorldPos.x,              (GLfloat)blockWorldPos.y, (GLfloat)(blockWorldPos.z - fullSize), texLeft, 1.0f,
            };
            break;
        case DOWN:
            texLeft = 1.0f * texWidth;
            texRight = 2.0f * texWidth;
            // bottom face (y = blockWorldPos.y - fullSize)
            tempVertices = {
                (GLfloat)blockWorldPos.x,              (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texLeft, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texRight, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z,              texRight, 1.0f,
                (GLfloat)blockWorldPos.x,              (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z,              texLeft, 1.0f,
            };
            break;
        case LEFT:
            texLeft = 2.0f * texWidth;
            texRight = 3.0f * texWidth;
            // left face (x = blockWorldPos.x) - at the left
            tempVertices = {
                (GLfloat)blockWorldPos.x, (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z,              texLeft, 0.0f,
                (GLfloat)blockWorldPos.x, (GLfloat)blockWorldPos.y,              (GLfloat)blockWorldPos.z,              texRight, 0.0f,
                (GLfloat)blockWorldPos.x, (GLfloat)blockWorldPos.y,              (GLfloat)(blockWorldPos.z - fullSize), texRight, 1.0f,
                (GLfloat)blockWorldPos.x, (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texLeft, 1.0f,
            };
            break;
        case RIGHT:
            texLeft = 3.0f * texWidth;
            texRight = 4.0f * texWidth;
            // right face (x = blockWorldPos.x + fullSize)
            tempVertices = {
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texLeft, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y,              (GLfloat)(blockWorldPos.z - fullSize), texRight, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y,              (GLfloat)blockWorldPos.z,              texRight, 1.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z,              texLeft, 1.0f,
            };
            break;
        case FORWARD:
            texLeft = 4.0f * texWidth;
            texRight = 5.0f * texWidth;
            // front face (z = blockWorldPos.z)
            tempVertices = {
                (GLfloat)blockWorldPos.x,              (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z, texLeft, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)blockWorldPos.z, texRight, 0.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y,              (GLfloat)blockWorldPos.z, texRight, 1.0f,
                (GLfloat)blockWorldPos.x,              (GLfloat)blockWorldPos.y,              (GLfloat)blockWorldPos.z, texLeft, 1.0f,
            };
            break;
        case BACKWARD:
            texLeft = 5.0f * texWidth;
            texRight = 6.0f * texWidth;
            // back face (z = blockWorldPos.z - fullSize) - at the back
            tempVertices = {
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texLeft, 0.0f,
                (GLfloat)blockWorldPos.x,              (GLfloat)(blockWorldPos.y - fullSize), (GLfloat)(blockWorldPos.z - fullSize), texRight, 0.0f,
                (GLfloat)blockWorldPos.x,              (GLfloat)blockWorldPos.y,              (GLfloat)(blockWorldPos.z - fullSize), texRight, 1.0f,
                (GLfloat)(blockWorldPos.x + fullSize), (GLfloat)blockWorldPos.y,              (GLfloat)(blockWorldPos.z - fullSize), texLeft, 1.0f,
            };
            break;
        }
        ...
        indicies.insert(indicies.end(), tempIndices.begin(), tempIndices.end());
    }
...
void Renderer::allocateMem(size_t sizeV, size_t sizeI) {
    ...
    //   only reserve if it needs more capacity than it haves
    if (indicies.capacity() < sizeI) {
        indicies.reserve(sizeI);
    }
